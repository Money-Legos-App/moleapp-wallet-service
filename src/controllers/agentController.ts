...
export const agentController = {
  ...
  createMission: async (req: Request, res: Response, next: NextFunction) => {
    try {
      const { userId, missionType, depositAmount, walletId, riskLevel } = req.body;
      const validRiskLevels = ['CONSERVATIVE', 'MODERATE'];
      const effectiveRiskLevel = validRiskLevels.includes(riskLevel) ? riskLevel : 'MODERATE';

      logger.info('Creating agent mission', { userId, missionType, depositAmount, walletId, riskLevel: effectiveRiskLevel });

      // Validate deposit amount bounds (defense-in-depth — agent-service also validates)
      const MIN_DEPOSIT_USDC = 10;
      const MAX_DEPOSIT_USDC = 10_000;
      const MAX_ACTIVE_MISSIONS_PER_USER = 5;

      const depositNum = parseFloat(depositAmount);
      if (isNaN(depositNum) || depositNum < MIN_DEPOSIT_USDC) {
      ...
      if (depositNum > MAX_DEPOSIT_USDC) {
        return res.status(400).json({
          success: false,
          error: 'INVALID_AMOUNT',
          message: `Maximum deposit is ${MAX_DEPOSIT_USDC} USDC`,
        });
      }

      // Rate limit: max concurrent active missions per user
      const activeMissionCount = await prisma.agentMission.count({
        where: {
          userId,
          status: { in: ['PENDING', 'DEPOSITING', 'APPROVING', 'ACTIVE'] },
        },
      });
      if (activeMissionCount >= MAX_ACTIVE_MISSIONS_PER_USER) {
        return res.status(429).json({
          success: false,
          error: 'RATE_LIMIT',
          message: `Maximum ${MAX_ACTIVE_MISSIONS_PER_USER} concurrent missions allowed`,
        });
      }
      ...
      // Max leverage derived from risk level
      const leverageByRisk: Record<string, number> = { CONSERVATIVE: 1, MODERATE: 2 };
      const maxLeverage = leverageByRisk[effectiveRiskLevel] || 2;

      // Agent key is generated by agent-service during Vault-based agent approval.
      // No key material is created or stored at mission creation time.

      // Lock the fee percentage at mission creation time (immutable after this).
      // Default: 30% of profit. Read from env to allow future flexibility,
      // but once locked per-mission it cannot change during the mission lifecycle.
      const PROFIT_FEE_PERCENT = parseFloat(process.env.PROFIT_FEE_PERCENT || '30');
      ...
      const mission = await prisma.agentMission.create({
        data: {
          ...
          maxLeverage,
          allowedAssets: (process.env.ALLOWED_ASSETS || 'BTC-USD,ETH-USD,SOL-USD,SUI-USD,SEI-USD,AVAX-USD,TAO-USD,FET-USD,NEAR-USD,WIF-USD,POPCAT-USD,kPEPE-USD,DOGE-USD,PENDLE-USD,ARB-USD').split(','),
          feePercent: PROFIT_FEE_PERCENT,
          status: 'PENDING',
        }
      });
  ...
  /**
   * Sign EIP-712 typed data using the mission's agent key.
   *
   * All agent key signing is now handled by agent-service via Vault Transit.
   * This endpoint is retained only to return a clear error directing callers
   * to the correct path.
   */
  signWithAgentKey: async (req: Request, res: Response, _next: NextFunction) => {
    const { missionId } = req.body;
    logger.warn('signWithAgentKey called — agent signing moved to agent-service Vault path', { missionId });
    return res.status(410).json({
      success: false,
      error: 'REMOVED',
      message: 'Agent key signing has moved to agent-service. Use the Vault Transit signing path.',
    });
  },

  /**
   * Batch sign — removed alongside single sign. All signing via agent-service Vault path.
   */
  batchSignWithAgentKey: async (req: Request, res: Response, _next: NextFunction) => {
    logger.warn('batchSignWithAgentKey called — agent signing moved to agent-service Vault path');
    return res.status(410).json({
      success: false,
      error: 'REMOVED',
      message: 'Batch agent key signing has moved to agent-service. Use the Vault Transit signing path.',
    });
  },
  ...
  depositToHyperliquid: async (req: Request, res: Response, next: NextFunction) => {
    try {
      ...
      if (!kernelAccount) {
        // Create Kernel account on Arbitrum Sepolia (shared singleton preserves caches)
        const { getKernelAccountFactory } = await import('../services/kernel/account-factory.singleton.js');
        const factory = getKernelAccountFactory();
